...
class Data_Loader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label_path = label_path.split('_')
        label_path.remove(label_path[-1])
        label_path = '_'.join(label_path)+'.png'
        #
        label_emb_path = image_path.replace('image', 'label')
        label_emb_path = label_emb_path.split('_')
        label_emb_path.remove(label_emb_path[-1])
        label_emb_path = '_'.join(label_emb_path) + '.png'
        #
...
if __name__ == "__main__":
    isbi_dataset = Data_Loader("D:\PENN\MC_6_2022_LSTM/data/")
    print("number：", len(isbi_dataset))
    train_loader = torch.utils.data.DataLoader(dataset=isbi_dataset,
                                               batch_size=10,
                                               shuffle=False)
    for image, label, label_emb in train_loader:
        print(image.shape, label.shape, label_emb.shape)
