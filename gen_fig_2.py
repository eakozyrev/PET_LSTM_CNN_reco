...
#        self.data_emb[:] = 0
    ...
    def kernel_function(self,x,y):
        d = self.gaussian(x,y,self.TOF_res,self.LOR_res,self.angle)
        return d
    ...
    def save(self,name,image_path):
        im = Image.fromarray(self.data1)
        im.save(image_path + name + ".png")

    def save_emb(self,number,image_path):
        im = Image.fromarray(self.data_emb)
        im.save(image_path + str(number) + ".png")

    def gen_plot(self, plotsize,):
        self.bckg = 133 + (np.random.rand(1).squeeze() - 0.5) * 70.
        self.clear();
        for j in range(int(np.random.rand(1)[0] * 200 / plotsize)):
            self.dr_rect(np.random.rand(5))
            self.dr_quadr(np.random.rand(4, 2))
            self.dr_circle(np.random.rand(5))
        for j in range(10):
            self.dr_embedded(np.random.rand(4))

    def draw_plot(self, path, i, j):
        self.kernel0 = self.kernel
        self.blurring()
        self.randomize()
        self.contour(1)
        self.save(f'{i}_{j}',path + "image/")
        self.contour(0)
        self.savedata(i, str(path + "label/"))
        self.save_emb(i, path + "label_emb/")


def generate_all(N,plotsize,path):
    plot = Toy_fig_2(144, 144)
    try:
        os.mkdir(path)
    except OSError:
        print('ciao1')
